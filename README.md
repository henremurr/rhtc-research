...
The first screen loads 25 illustrative symbols without a token. With a sandbox token, scans are capped at 10 symbols per request to reduce the chance of exceeding Tradier's 60 market-data requests per minute; use ticker search or Peak filters to narrow the scan. The 127-symbol universe is available in the watchlist. Focus symbols are a curated research subset, not a verified position ledger.

Set `TRADIER_API_TOKEN` in the server environment to request Tradier market data. Set `OPENAI_API_KEY` to enable a short research note grounded in the displayed rows; without it, the note uses deterministic text. `OPENAI_MODEL` optionally overrides the default model. Never commit tokens. No quote or trade action is submitted to a broker.
